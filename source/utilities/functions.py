...
async def get_group_help(ctx: context.CustomContext, group: commands.Group):
    async def check(interaction: discord.Interaction) -> None:
        return interaction.user.id == ctx.author.id

    paginator = buttons.Paginator(ctx, embeds=[], timeout=20.0, check=check)
...
async def paste(bot: bot.Bot, text: str):

    data = bytes(text, encoding="utf-8")
    async with bot.session.post(url=MYSTB_DOCUMENTS, data=data) as response:

        if response.status != 200:
            raise errors.ProcessError(f"Unexpected error with return status {response.status}")
...
async def error_handler(ctx: context.CustomContext, error: commands.CommandError):
    bot = ctx.bot
    ...
    if isinstance(error, commands.MissingRequiredArgument):
        ...
        isinstance(error, errors.ProcessError)
        or isinstance(new_error, errors.ProcessError)
        or type(error) is errors.ProcessError
        ...
        async with bot.pool.acquire(timeout=constants.Time.db_time) as conn:
        ...
        embed.description = f"Unknown error. Please report it in [support server]({constants.General.support_guild_invite}).\n**Bug id:** {bug_id}"
