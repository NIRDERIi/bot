...
class Tags(commands.Cog):
    ...
    async def tag(self, ctx: CustomContext, *, name: str) -> None:
        async with self.bot.pool.acquire(timeout=Time.db_time) as conn:
            ...
            if not results:
                return await ctx.send(f"{Emojis.custom_denial} No such tag named: `{name}`.")
    ...
    async def create(self, ctx: CustomContext, name: str, *, content: str):
        async with self.bot.pool.acquire(timeout=Time.db_time) as conn:
            ...
            if not name.isascii:
                return await ctx.send(f"{Emojis.custom_denial} Must not have non-ascii characters in tag name.")
            ...
            results = await conn.execute(query, ctx.guild.id, ctx.author.id, name, content, 0, datetime.datetime.now(), datetime.datetime.now())
            await ctx.send(f'{Emojis.custom_approval} Succesfully created tag `{name}`.')
